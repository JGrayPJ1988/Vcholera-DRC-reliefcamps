...
calculateBF <- function(logfile, burninpercentage, locations, traitname) {
  print("Loading data...")
  N_traits <- length(locations)
  locations <- as.data.frame(locations)
  burnin <- as.numeric(burninpercentage)
  
  # Read log file
  log <- read.table(logfile, header = TRUE, check.names = FALSE)
  
  # Print column names for debugging
  print("Column names in log file:")
  print(names(log))
  
  # Remove burn-in
  burnin_rows <- round(nrow(log) * burnin / 100)
  if (burnin_rows >= nrow(log)) {
    stop("Error: Burn-in percentage is too high; no data remains.")
  }
  log <- log[(burnin_rows + 1):nrow(log), ]
  
  print("Creating empty results dataframe...")
  df <- tibble("Transition" = character(), "BayesFactor" = numeric())
  
  print("Calculating Bayes Factors...")
  for (i in 1:nrow(locations)) {
    for (j in 1:nrow(locations)) {
      if (i != j) {
        # Generate indicator label in different formats
        indicatorlabel_dot <- sprintf("%s.indicators.%s.%s", traitname, locations[i,], locations[j,])
        indicatorlabel_underscore <- gsub("\\.", "_", indicatorlabel_dot)
        
        # Try to find the column in log file
        index1 <- match(indicatorlabel_dot, names(log))
        if (is.na(index1)) {
          index1 <- match(indicatorlabel_underscore, names(log))
        }
        
        # Skip if column is missing
        if (is.na(index1)) {
          warning(sprintf("Column %s not found in log file. Skipping...", indicatorlabel_dot))
          next
        }
        
        # Compute Bayes Factor
        p <- sum(log[, index1] == 1) / nrow(log)
        K <- N_traits  # Adjust for symmetric case if needed
        q <- (log(2) + K - 1) / (K * (K - 1))
        BF <- (p / (1 - p)) / (q / (1 - q))
        
        # Save result
        label <- sprintf("%s.to.%s", locations[i,], locations[j,])
  ...
  # Save results
  output_file <- sprintf("%s.BFs.csv", traitname)
  write.csv(df, file = output_file, row.names = FALSE)
  
  print(sprintf("Bayes Factor approximation calculated and saved as %s", output_file))
}
########################
